...
### 🔧 Adaptation Changes
- **Public Headers and Module Map**: A dedicated `Public` folder was introduced to support Swift Package Manager without changing the overall folder structure. This folder aggregates the public header files from both the `Classes` and `Internal` directories using symbolic links, and it contains a custom `module.modulemap` file that instructs SPM how to build the module.

### 📦 Dependency Status
This library is a core dependency for the Speak iOS application. It handles all analytics tracking and event reporting within the app.

### ⚙️ Integration Notes
- Do not update this library without thorough testing of analytics events in the main application
- The main Speak app relies on specific event tracking methods provided by this library
- Any changes to this dependency may affect analytics reporting across the entire application
